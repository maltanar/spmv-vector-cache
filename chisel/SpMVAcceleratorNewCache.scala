...
class SpMVAcceleratorNewCache(p: SpMVAccelWrapperParams) extends AXIWrappableAccel(p) {
  override lazy val accelVersion: String = "beta-2"
  ...
  // plug backend output vec input
  backend.outputVecIn.valid := Bool(false)
  backend.outputVecIn.bits := UInt(0)
  ...
  override def defaultTest(t: WrappableAccelTester): Boolean = {
    ...
    def doneWrite(): Boolean = {t.readReg("out_statFrontend") == 2}
    ...
    def spmvWrite() = {
      ...
      t.hooks("traceWrite") = traceWrite
      while(!doneWrite()) {}
      t.hooks.remove("traceWrite")
    ...
    def traceRegular() = {
      ...
      //printWhenTransactionAggr("$rreq", frontendM.cache.read.req)
      //printWhenTransactionAggr("$rrsp", frontendM.cache.read.rsp)
    ...
    def traceWrite() = {
      //printWhenTransactionAggr("req", frontendM.io.mp.memWrReq)

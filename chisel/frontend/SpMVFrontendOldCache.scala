...
// frontend using the old (original) vector cache from the ARC paper

class SpMVFrontendOldCache(val p: SpMVAccelWrapperParams) extends Module {
  ...
  */
  ...
  val cacheM = Module(new SimpleDMVectorCache(p))
  val cache = cacheM.io

#ifndef SpMVAcceleratorOldCacheDriver_H
#define SpMVAcceleratorOldCacheDriver_H
#include <assert.h>
class SpMVAcceleratorOldCacheDriver {
public:
 static unsigned int expSignature() {return 0x44b87990;};
 SpMVAcceleratorOldCacheDriver(volatile unsigned int * baseAddr) {
  m_baseAddr = baseAddr; assert(signature() == expSignature());};
 // read+write register: startInit index: 11
 void startInit(unsigned int v) {m_baseAddr[11] = v;};
 unsigned int startInit() {return m_baseAddr[11];};
 // read+write register: startRegular index: 12
 void startRegular(unsigned int v) {m_baseAddr[12] = v;};
 unsigned int startRegular() {return m_baseAddr[12];};
 // read+write register: startWrite index: 13
 void startWrite(unsigned int v) {m_baseAddr[13] = v;};
 unsigned int startWrite() {return m_baseAddr[13];};
